...
## Acceptance criteria
- [ ] [REQ-001] Timeout is a supervisor config value (not hard-coded); `docs/spec/configuration.md` documents it in the same commit
- [ ] [REQ-002] An in-box run that exceeds the timeout triggers a box kill and deterministic teardown (the 017 teardown guarantee still holds)
- [ ] [REQ-003] The run outcome records `timed-out`, distinguishable from a gate failure or a normal completion
...
- **L5:** `go test ./internal/supervisor/...` — fake loop blocks past a short configured timeout; assert box killed + outcome == timed-out
- **L6:** operator-observed log line showing the timeout fired and the box was torn down
...
- Updates `docs/spec/configuration.md` in the same commit (new timeout config). Do not edit spec during backlog authoring.
