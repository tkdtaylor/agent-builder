...
- Related ADRs: ADR required: agent-loop state-machine shape (states + outcome type)
...
- [ ] Blocking tasks complete: 002, 010

## Acceptance criteria
- [ ] [REQ-001] A loop type advances through pick → attempt → verify → advance; state transitions are observable in test
- [ ] [REQ-002] With a fake Executor returning a branch and a fake Gate returning pass, the loop emits a done outcome carrying that branch
- [ ] [REQ-003] With a fake Gate returning fail, the loop emits a fail outcome (no retry decision made in the loop) and the cycle is suspended pending the policy
...
- Harness: `go test ./internal/loop/...`. Expected final assertion: on pass, outcome == done with the expected branch; on fail, outcome == fail with no retry count set.
...
- `Executor.Run(Task)(Result,error)` and `Gate.Verify(repoPath)(bool,error)` are the consumed seams.
