...
| 018 | Wall-clock timeout / runaway kill | 018-wall-clock-kill-test-spec.md | ✅ | 🟡 | L5: `go test -count=1 -v ./internal/supervisor -run 'TestRunTimeout(UsesConfiguredDeadlineAndKillsBox\|RecordsTimedOutOutcome)'` -> `event=box.kill.timeout` + `"outcome":"timed-out"`; L3: `env PATH=/tmp/agent-builder-tools:$PATH make check` -> `All checks passed.`; `make fitness` -> `Fitness checks passed.` |
