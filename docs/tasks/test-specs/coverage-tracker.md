...
| 016 | Tiered OCI runtime selection seam | 016-tiered-runtime-seam-test-spec.md | ✅ | 🟡 | L3: `env PATH=/tmp/agent-builder-tools:$PATH make check` -> `All checks passed.`; L6 probe blocked locally: `containment/execution-box/run.sh --worktree . --runtime runsc --probe` -> `execution-box: podman unavailable on PATH` |
...
| 021 | sandbox-runtime backing adapter | 021-sandbox-runtime-adapter-test-spec.md | ✅ | 🟡 | L3: `env PATH=/tmp/agent-builder-tools:$PATH make check` -> `All checks passed.`; L6 blocked locally: `command -v srt` -> exit 1/no output; `command -v bwrap` -> `/usr/bin/bwrap` |
