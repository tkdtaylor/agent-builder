...
| 013 | Escalation + retry-N + stop condition | 013-escalation-retry-policy-test-spec.md | ✅ | 🟡 | L5: `go test ./tests/loop/... -run 'TestRetryPolicy|TestEscalation' -count=1` -> `ok  	github.com/tkdtaylor/agent-builder/tests/loop	0.002s`; `env PATH=/tmp/agent-builder-tools:$PATH make check` -> `All checks passed.`; `make fitness` -> `Fitness checks passed.` |
