...
| 010 | Roadmap task-source reader (read-only) | 010-roadmap-task-source-test-spec.md | ✅ | 🟡 | `go test ./internal/tasksource/...` -> `ok github.com/tkdtaylor/agent-builder/internal/tasksource`; `env PATH=/tmp/agent-builder-tools:$PATH make check` -> `All checks passed.` |
