...
|---------|---------|-----------|---------------|--------|-------------|
| 001 | Walking skeleton & project setup | 001-walking-skeleton-test-spec.md | ✅ | 🟡 | Task 031 spec-verifier BLOCK: TC-002 is stale because it names the old `TestRunNotYetImplemented` / `Supervisor.Run() returns ErrNotImplemented` stub, while the current supervisor dispatches configured containment/loop seams; TC-003 is stale because bare `go run ./cmd/agent-builder` now prints CLI usage and exits 2, with the version banner under `agent-builder version`. |
| 002 | Gate orchestrator core + Verdict model | 002-gate-orchestrator-core-test-spec.md | ✅ | ✅ | Task 031 spec-verifier APPROVE + L2 unit-test-only/no runtime surface: `go test -count=1 -v ./internal/gate -run 'TestVerify\|TestNewRejects\|TestGateHasNoSkipSurface'` -> `ok github.com/tkdtaylor/agent-builder/internal/gate 0.002s` |
...
| 026 | armor on web-ingestion / tool-call path | 026-armor-ingestion-wiring-test-spec.md | ✅ | ✅ | Task 031 spec-verifier APPROVE + L5: `go test -count=1 -v ./tests/executorharness -run 'TestHarnessProducerConsumerTraceCoversLivePath\|TestArmorGuardedHarnessProducerConsumerTraceCoversLiveExecutorPath'` -> `TC-005 armor producer-consumer trace:` |
