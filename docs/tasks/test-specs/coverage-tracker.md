...
|---------|---------|-----------|---------------|--------|-------------|
| 001 | Walking skeleton & project setup | 001-walking-skeleton-test-spec.md | ✅ | 🟡 | Spec corrected (audit 2026-06-05): TC-002/TC-003 rewritten to current truth — `Run()` dispatch (superseded by task 017) and the `run`/`version`/`verify` CLI surface (superseded by task 023) now documented with provenance notes; the task-031 spec-verifier BLOCK is cleared. Pending re-verify commit for L6 evidence. |
