...
| 035 | Podman sandbox.Runner adapter | 035-podman-sandbox-adapter-test-spec.md | ✅ | 🟡 | L3: `env PATH=/tmp/agent-builder-tools:$PATH make check` -> `ok github.com/tkdtaylor/agent-builder/internal/sandbox/podman 5.015s`; unit-test-only; no runtime surface (L6 harness would require real Podman + runsc on host) |
