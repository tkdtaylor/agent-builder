...
| 012 | Agent loop state machine | 012-agent-loop-test-spec.md | ✅ | 🟡 | `env PATH=/tmp/agent-builder-tools:$PATH make check` -> `All checks passed.`; `make fitness` -> `Fitness checks passed.`; L5: `go test -count=1 ./tests/loop/...` -> `ok github.com/tkdtaylor/agent-builder/tests/loop` |
