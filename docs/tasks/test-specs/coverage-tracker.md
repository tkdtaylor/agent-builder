...
| 009 | Fitness F-002: gate is blocking (no skip) | 009-fitness-gate-blocking-test-spec.md | ✅ | 🟡 | `make fitness-gate-blocking` -> `PASS fitness-gate-blocking: no verification gate bypass affordances found.` Negative fixtures caught CLI flag, scanner skip env-var, and early-return bypass; `env PATH=/tmp/agent-builder-tools:$PATH make check` -> `All checks passed.` |
