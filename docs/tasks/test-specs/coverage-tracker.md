...
| 019 | Run log collection (audit-trail seam) | 019-run-log-collection-test-spec.md | ✅ | 🟡 | L5: `go test -v ./tests/supervisor -run TestRunRecordStreamsOutputAndPersistsAfterTeardown -count=1` -> `TC-003-Persist-After-Teardown sample persisted line: {"box_id":"box-019"` |
