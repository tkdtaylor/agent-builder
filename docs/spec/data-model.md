...
Reason            string    free-text reason for rejection or diagnostic events, e.g. "unknown_key", "replay_detected", "decryption_failed", "armor_blocked", "role_mismatch" (channel-reject events; task 080), "policy_transport_error" (policy-decision events on a Decide transport/parse failure; task 166)
```

- **Identity:** embedded in `AuditEvent`; carries only the non-zero fields relevant to the action. `PolicyDecision` and `PolicyReason` are set only for `ActionPolicyDecision` events; `Reason` is set for `ActionChannelReject` events and, since task 166, for an `ActionPolicyDecision` event emitted on a Decide transport/parse failure (`"policy_transport_error"`, distinguishing a fail-closed deny from a policy-authored deny); it is zero-valued on all other event types.
