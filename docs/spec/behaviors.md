...
- **Response:** the mode decision (owned by `internal/channel/telegram/authz`) returns one of four actions per update:
  ...
  - **`pairing`:** an approved sender's plaintext routes exactly as `allowlist` (same armor + derive pipeline); an **unknown** sender enters the owner-approved in-chat flow (B-039). `open` is recognized by config validation (task 151); its runtime accept path lands in task 153.
