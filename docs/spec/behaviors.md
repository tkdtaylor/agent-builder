...
- **Response:** the loop dispatches by `Kind`. `MsgNewGoal` drives the actor-per-goal flow (B-027). `MsgStatus` is routed to the status handler (the handler body — registry render + immediate Reporter answer — is task 114; an empty GoalID is a fleet query). `MsgInfo`/`MsgCancel` are delivered to the **addressed goal's per-goal command mailbox** — a buffered `chan supervisor.Message` keyed by goalID, created **before** the goal actor is registered/started (register-then-start), so an `info`/`cancel` arriving at actor startup is delivered, not lost or raced.
