...
- **Response:** The supervisor records a timeout error, emits a structured `box.kill.timeout` log event, calls `Kill` on the created containment box, writes a terminal run-record outcome of `timed-out` when run-record collection is configured, and tears the box down exactly once. The run-loop `select` always **joins the in-box loop goroutine** before returning on the kill path (no goroutine leak / no orphaned writer).
...
- **Relationship to cancellation (B-031):** the wall-clock timeout is **one** trigger into this kill/teardown path; context cancellation is a **second** trigger into the **same** path (B-031). The timeout remains the independent backstop even when a cancel races it.
