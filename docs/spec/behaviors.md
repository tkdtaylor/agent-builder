...
- **Trigger:** the `orchestrate` subcommand's control loop reads a goal from the `GoalSource`. (The loop is the single, sole reader of the source — no concurrent `Next()` races.)
- **Response:** for each goal the loop **registers a `Queued` entry** in the live status registry, then **spawns a goal-actor goroutine** that owns that goal's lifecycle by calling `Orchestrator.Handle`. Reading the next goal and processing a goal are **decoupled**: a goal held in `Dispatching` does not stall intake of the next, and M goals run concurrently. The actor first acquires a **goal-admission slot** (`AGENT_BUILDER_MAX_GOALS`); while the fleet is at the admission cap the goal stays `Queued` (Handle has not run) until a live goal terminates and frees a slot. Inside dispatch, each per-sub-goal goroutine acquires one permit from the **fleet-wide worker semaphore** (`AGENT_BUILDER_MAX_WORKERS`) before the worker dispatch and releases it (deferred) after, so total live workers across all goals never exceeds that bound. The loop reads until the source is exhausted (`ok=false`) or errors, then joins every spawned actor before returning.
...
- **References:** ADR 054 §1/§3/§6; task 112; `docs/tasks/test-specs/112-async-control-loop-core-test-spec.md`. The typed message protocol (`status`/`info`/`cancel`), the immediate status-query answer, apply-info-at-checkpoint, and cancellation are deferred to tasks 113–116; this behavior reads goals only.
