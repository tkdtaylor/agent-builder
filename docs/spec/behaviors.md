...
- **Response:** the actor fires the goal's `CancelFunc`, which cancels ONLY that goal's derived context. The cancellation propagates to the worker seam: a dispatched worker's `Supervisor.Run` selects the run-loop's `case <-ctx.Done():` arm and runs the **same** `box.Kill`/`Teardown` path the wall-clock timeout drives (B-013) — no new teardown mechanism is invented — returning `supervisor.ErrRunCancelled`. The actor then sets the goal `Cancelled`, consumes the plan from the `PlanStore` under the **same delete path** `Resume` uses (so a racing approval cannot resurrect or double-dispatch it — ADR 054 §6 race (d)), and reports the cancellation.
