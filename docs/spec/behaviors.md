...
- **Failure modes:** a cancel for an unknown goalID is answered with the router's graceful "no such goal" report (B-028) — never a panic, no mailbox auto-created. A partial-teardown failure (a worker whose `box.Kill` errors) surfaces the joined kill error in the worker's sub-goal outcome (recorded `failed`, the leak named in the goal's `PlanResult` report — not swallowed); the wall-clock timeout remains the backstop. Cancelling an already-terminal goal is a no-op on the worker side (no live ctx to cancel) but still records `Cancelled`. A cancel racing a `Resume`-approve consumes the plan exactly once: only `Resume` dispatches, and it dispatches at most once per consumed plan — the dispatch is called at most once total across the pair (no double-dispatch).
