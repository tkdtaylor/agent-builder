...
| agent-builder CLI | Default Run Wiring | `internal/runtime` | Host-side CLI bootstrap that parses explicit run configuration, selects one ready task, composes the concrete run adapters, and hands one configured task to the Supervisor | Supervisor; Task Source; Task Status Writer; Agent Loop; Claude CLI Executor; Verification Gate; Podman Adapter; Branch Publisher |
