...
- `--probe` runs the containment probe and prints `TC-001` through `TC-005` PASS/FAIL output plus host-side resource inspection for `TC-003` (load-bearing CPU/memory/PID/SHM limits always checked; StorageOpt checked only when quota is enforceable on this host — see ADR 027), host-side runtime inspection for `TC-016`, in-box `TC-016-RUNTIME` output, and Gate tool path/version evidence for `go`, `gofmt`, `golangci-lint`, `gods`, and `code-scanner`. When the runtime is `runsc`, the probe also runs a trivial `go build` and prints `TC-016-GO`. On non-XFS hosts, `TC-003 PASS` is printed with a note that storage quota is not enforced; `TC-003 FAIL` is still printed when the load-bearing CPU/memory/PID/SHM limits are absent.
