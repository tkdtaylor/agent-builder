...
| `daemon` | subcommand | — | Long-lived control-plane process (task 174, ADR 065). Reuses `orchestrate`'s `assembleOrchestrate`/`runControlLoop` verbatim (no new control-loop logic) with three lifecycle additions: a single-instance lock file (`AGENT_BUILDER_DAEMON_LOCK`, `O_CREATE\|O_EXCL` PID file) that fails a second concurrent instance fast; graceful shutdown on SIGINT/SIGTERM via `signal.NotifyContext` (cancels the shared control-loop context, then returns `0` after cleanup, removing the lock); and startup rehydration of in-flight runs (`RehydrateInFlight`/`ResumeFromRecord`) before the steady-state loop when `AGENT_BUILDER_RUN_STORE_DIR` is set. The lock is removed on graceful shutdown AND on any startup-assembly error. Every `AGENT_BUILDER_*` env var `orchestrate` reads applies identically. Returns `0` on graceful shutdown, `1` on a lock/assembly/loop failure, `2` on usage error. |
| `orchestrate` | subcommand | — | Drives the Tier-1 orchestrator (ADR 042/046/048/049/050). Assembles the orchestrator stack from environment — `NewPlanStoreFromEnv` (memory-guard-backed or in-memory + warning), the worker envelope transport with **one shared `ReplayCache` per direction** (083 SEC-001), the planner selected by `AGENT_BUILDER_PLANNER` (`structured` default rule-based StructuredPlanner; `llm` LLM-backed LLMPlanner via router/registry — ollama-native only, cloud harnesses fail closed; task 110/ADR 053), the policy client, and **one shared `audit.Sink`** between orchestrator and dispatched workers (ADR 050 §4) — then runs the **async control loop** (ADR 054 §1): a single goroutine reads typed messages from a `supervisor.MessageSource` (the line-oriented env/stdin source by default — see [configuration.md](configuration.md#stdin-command-grammar-orchestrate) for the grammar) and **routes each by kind** (`new-goal` → goal actor via `Handle`; `status` → status handler; `info`/`cancel`/`confirm` → the addressed goal's command mailbox, where the goal actor applies info-at-checkpoint (task 115) or fires the goal's `CancelFunc` to tear down its in-flight workers (task 116); unknown goalID → graceful "no such goal" report). **Fail-closed at startup (083 SEC-003):** a missing/unreadable `AGENT_BUILDER_WORKER_SIGNING_KEY` exits non-zero with an error satisfying `errors.Is(err, worker.ErrMissingSigningKey)` **before any message is read**. Returns `0` on a clean run (source drains to EOF and all actors join), `1` on assembly or handling failure, `2` on usage error. |
| `ask [--entry <id>] <prompt>` | subcommand + optional flag + args | — | The general (non-coding) entrypoint (ADR 059). Selects a registry brain — explicit `--entry <id>` (resolved from `registry.LoadFromEnv`; unknown id → `2`), else the router's default selection (synthetic default Claude entry when the registry is empty) — constructs its single-shot `executor.Completer`, joins the trailing args into the prompt, and prints the trimmed answer to stdout (`0`). No worktree, no verification gate, no branch — read-only inference. Empty prompt → `2`; a `CompleterForEntry` error (unsupported harness, e.g. codex/gemini) or a `Complete` error → `1` with the error on stderr and nothing on stdout. |
...
- **Auth contract (cloud mode):** when `BaseURL` is empty (cloud entry), the executor accepts either `ANTHROPIC_API_KEY` or `CLAUDE_CODE_OAUTH_TOKEN` (OAuth token preferred when both are set). Exactly one credential is injected into subprocess env; the other is stripped. Host `HOME`/XDG dirs are replaced with temp dirs. Both credential values are redacted from subprocess failure output. The executor fails before subprocess start when both are absent. **For the single-shot `Completer` path (task 143, ADR 059):** when both env-var credentials are absent, the OAuth token is read from `${HOME}/.claude/.credentials.json` (the on-disk Claude CLI subscription login, before HOME is isolated); only `claudeAiOauth.accessToken` is read, never the refresh token. The disk credential is consulted only after both env vars are checked and found empty — explicit env tokens are never overridden by disk. Graceful absence: missing/malformed/empty disk file → empty token, no error (no regression when there is no on-disk login).
...
> **This is the non-coding execution path (ADR 059).** Every `supervisor.Executor` harness is coding-shaped (edits a worktree, returns a gate-verified branch). The single-shot `Completer` answers without editing a repo and is supported for the **three live brains** — `HarnessOllamaNative`, `HarnessClaudeCLI` (`claude -p`), and `HarnessAntigravityCLI` (`agy --print`). `HarnessCodexCLI` and the deprecated `HarnessGeminiCLI` still fail closed with `ErrSingleShotUnsupported`. The `ask` CLI subcommand is the human entrypoint onto this path.
...
- **Implementors:** `*ollamaCompleter` (ollama-native; wraps `ollamaclient.Chat`), `*claudeCompleter` (`claude -p`; auth mirrors `NewClaudeCLIFromEntry`), `*antigravityCompleter` (`agy --print --model`; subscription/OAuth) — ADR 059.
...
- **Dispatch contract (`CompleterForEntry`):**
  ...
  - `HarnessClaudeCLI` → `claudeCompleter` (runs `claude -p <prompt>`; ADR 059); `HarnessAntigravityCLI` → `antigravityCompleter` (runs `agy --print <prompt> --model <model>`; ADR 059).
  - `HarnessCodexCLI` / `HarnessGeminiCLI` (deprecated) / unknown harness → `nil, fmt.Errorf("harness %q … not yet supported: %w", ErrSingleShotUnsupported)` (fail-closed; never silently wrong — ADR 053 §2).
...
- **Consumers:** runtime/dispatcher wiring (task 095) resolves a recipe's routing spec to a concrete executor through this router. As of task 160, `runtime.resolveExecutor` **retains** the `*router.Router` it constructs (instead of discarding it after the initial `Select`) and returns it to `runtime.Run`, which builds a router-backed `agentloop.EscalationHook` (`runtime.newRouterEscalationHook`) that drives `OnGateFailure` + `Select` across the bounded retry loop — so a gate-failing attempt climbs the capability ladder on the live `run`/`orchestrate` path rather than blindly retrying the same executor.
...
- **Required behavior:**
  ...
  - **Gate-failure escalation (quality axis):** `OnGateFailure(entryID)` records the entry as tried-and-failed for the current dispatch, so the next `Select` returns the next-stronger eligible entry. Once every eligible entry has gate-failed, `Select` returns `ErrNoEligibleExecutor`. Does not touch the entry's availability. `ResetEscalation()` clears this set for a fresh dispatch (availability state is unaffected). This axis is wired into the live retry/escalation hook as of task 160 (`runtime.newRouterEscalationHook`): each failed non-terminal attempt calls `OnGateFailure(currentEntryID)` then re-`Select`s, and when `Select` returns `ErrNoEligibleExecutor` the hook returns the current executor unchanged (graceful degradation), never a hard error out of the retry loop. The escalation set is per-`Router`, and each dispatch (`resolveExecutor` call) gets a fresh `Router`, so state does not leak across independent task dispatches.
...
- **Required behavior:** `Run` dispatches exactly one configured task per call. It creates a box before starting the in-box loop, passes the created `BoxHandle`, task, and host-side stream writers to the loop, and tears the box down exactly once after the loop returns, panics, or exceeds a configured timeout. Missing task, box, or loop dependencies fail before creation. Loop errors and recovered panics are returned after teardown. When `WithRunTimeout` receives a positive duration and the in-box loop exceeds it, the supervisor **cancels the run-scoped child context it threaded into `RunInside`** (task 156), calls `Kill` on the created box, records a timed-out run outcome, then tears down the box; kill errors are joined into the returned error and do not skip teardown. Non-positive timeouts leave the timeout disabled.

  **How the in-flight run is actually terminated (tasks 155–156):** `Run` derives a cancellable child (`runCtx`) of the caller `ctx` and threads `runCtx` — not the raw `ctx` — into `RunInside`. Both termination triggers cancel that **same** `runCtx`: a caller cancel cascades to it automatically (it is a child of `ctx`), and the wall-clock timeout arm calls its cancel function explicitly. A context-aware `Executor.Run` observes the cancellation and terminates its subprocess (`exec.CommandContext`) / HTTP call, which is what unblocks `RunInside` and lets `killAndJoin` complete. The production `ContainmentBox` (`runtime.sandboxBox`) `Kill`/`Teardown` are **intentional no-ops**: no separately-killable box-level process exists in the current architecture (the CLI-shaped executors spawn their subprocess directly), so real termination rides the cancelled context, not a box-level kill. A future real containment box would put its process kill in `Kill`. When `WithRunRecordPath` is configured, stdout/stderr/command writes are persisted as RunRecord NDJSON during the run, the terminal outcome is written, and the file is closed before teardown. Retry and escalation behavior remain outside this seam.
