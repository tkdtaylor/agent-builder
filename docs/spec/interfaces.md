...
- **Egress allowlist contract:** each allowlist entry is written to a temporary file in `host:port # comment` format; non-commented entries cause the launcher to reject the allowlist. Zero memory or CPU leave the corresponding env var unset (launcher uses its defaults).
