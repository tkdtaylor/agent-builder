...
**Last updated:** 2026-07-01 (drift audit — Fitness list completed to F-001…F-014; prior: 2026-06-30 goal-alignment — general-agent framing, agy as 3rd brain, gemini deprecated)
...
- **F-004 — default run pipeline has no sandbox-runtime dependency:** implemented by `make fitness-no-srt`; `internal/runtime`'s transitive imports contain no `sandboxruntime` package (the ADR 021 srt-removal, enforced by absence rather than deletion).
- **F-005 — audit leaf and its supervisor wiring have no executor/LLM/web or audit-trail-module dependency:** implemented by `make fitness-audit-isolation`; neither `internal/audit` nor `internal/supervisor` acquires an executor/LLM/web-fetch path or an `audit-trail` Go-module import (ADR 026 Option A).
- **F-006 — policy is a leaf and runtime reaches policy-engine only over IPC:** implemented by `make fitness-policy-isolation`; `internal/policy` imports no other `internal/` package and `internal/runtime` carries no `github.com/tkdtaylor/policy-engine` Go-module import (ADR 038 out-of-process invariant).
- **F-007 — envelope is a pure leaf absent from the supervisor graph:** implemented by `make fitness-envelope-isolation`; `internal/envelope` imports only stdlib + `golang.org/x/crypto`, and `internal/supervisor` never imports it (ADR 045).
- **F-008 — default run pipeline wires the exec-sandbox block backend:** implemented by `make fitness-exec-sandbox-default`; `internal/runtime`'s dependency graph includes `internal/sandbox/execsandbox` (ADR 035 self-leveraging swap).
- **F-009 — Mermaid diagrams render on GitHub:** implemented by `make fitness-diagrams-render`; `README.md` and every `docs/**/*.md` Mermaid block is free of GitHub-parser render hazards.
