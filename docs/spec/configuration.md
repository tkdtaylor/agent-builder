...
**Last updated:** 2026-06-29 (task 117 — AGENT_BUILDER_INBOUND=telegram and AGENT_BUILDER_TELEGRAM_* vars added; Telegram inbound+outbound now selectable at runtime)
...
| `AGENT_BUILDER_MAX_ATTEMPTS` | non-negative integer | none | yes for `agent-builder run`; optional for `agent-builder orchestrate` | Explicit bounded retry attempt count for the selected task (`agent-builder run`) or for blocked-action reevaluation before human escalation (`agent-builder orchestrate`, ADR 055 seam 4, task 123) |
