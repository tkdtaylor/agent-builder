...
| `CLAUDE_CODE_OAUTH_TOKEN` | secret string | none | no (one of two for cloud entries); fallback to disk when unset | Subscription OAuth token alternative to `ANTHROPIC_API_KEY`. When set in the process environment, it is preferred over the API key and injected into the subprocess environment for cloud entries. Minted by `claude setup-token` on a Claude Pro/Max subscription. **Disk fallback (task 143, ADR 059):** when the env var is absent AND the `ask --entry claude-oauth` completer path is used, the token is read from `${HOME}/.claude/.credentials.json` (the on-disk Claude subscription login; only `claudeAiOauth.accessToken` is read, never the refresh token). The disk credential is consulted only when both env-var credentials are empty — explicit env tokens are never overridden by disk. Graceful absence: missing/malformed/empty disk file → empty token, no error (backwards-compatible fallback for hosts without on-disk login). Never injected for local entries (empty `SecretRef`). The executor fails before subprocess start when both the env token and the disk token are absent for cloud entries and no API key is set. |
...
| `AGENT_BUILDER_GOAL_ANALYSIS` | enum string | (unset = off) | no | **`orchestrate` subcommand only** (ADR 060). When enabled with `heuristic`, `true`/`1`/`yes`/`on` (case-insensitive), or `llm` (case-insensitive, task 142), the orchestrator analyzes each goal at intake and classifies its **kind** (answer vs coding) and **complexity**. `heuristic` (or any truthy string except `llm`) selects the deterministic rule-based analyzer; `llm` selects the LLM-backed analyzer (fallback to heuristic on parse/invoke error, never breaking intake). A `KindAnswer` goal (a general question, no repo) is **answered over the channel** via the single-shot Completer — routed to a brain at a capability floor derived from complexity — and never dispatched (read-only inference: no worker, gate, branch, spawn-policy, or approval). A `KindCoding` goal follows the existing clarify→plan→gate→approval→dispatch flow. A `KindAnswer` goal is **multi-turn** (ADR 060 §6): after the first reply it lingers in `StateConversing`, and a follow-up `info <goalID> <text>` is answered with the running conversation transcript as context, until `cancel <goalID>` or source EOF ends it. When unset/false (default), every goal is coding (pre-060 behavior). |
...
| `AGENT_BUILDER_ROUTER_STATE_PATH` | path | unset (in-memory only) | no | Plain-text (JSON) file the Model Router's per-entry `Usage`/`Availability` quota state is persisted to and loaded from across successive `agent-builder run`/`orchestrate` process invocations (ADR 043, task 162). **When set:** `resolveExecutor` calls `router.LoadState(path)` immediately after constructing the router and before the first `Select` — a missing file (first run; nothing persisted yet) is tolerated, never an error, and never creates the file. A corrupted/malformed file IS a fail-fast `run`/`resolveExecutor` error, before any sandbox creation — the router never silently resets to fresh state on corruption. After the dispatch's `RecordDispatch`/`OnGateFailure` calls complete (inside `supervisor.Run`), `Run` calls `router.SaveState(path)` — this fires regardless of whether the dispatch itself succeeded, since usage was already recorded against the router's in-memory state either way. **When unset (default):** behavior is byte-for-byte identical to pre-task — the router is in-memory only and its quota state resets on every process invocation; no file is ever read or written. |
...
| `AGENT_BUILDER_MEMORY_GUARD_BIN` | path/name | unset | no | Path to the `memory-guard` block binary. **When set, the memory-guard write-gate + delete-verify + read-gate are enabled** (ADR 049 task 084; ADR 065 task 173): orchestrator plan state writes go through `validate_write` (fail-closed on allow=false), deletes go through `verify_delete` (tamper=halt), and reads (`PlanStore.Get`) go through `validate_read` (fail-closed: a denied read returns "not found", never the plan). Since task 173 the plan store is also **durable** (crash-safe on-disk journal), so a plan awaiting approval survives a restart. When unset, the orchestrator degrades gracefully to the in-memory `MemoryPlanStore` (v1) and emits a structured warning log naming this variable. The absence of this variable is never a startup error. A set-but-unresolvable binary surfaces as an error on the first IPC call; a malformed on-disk plan-store state is a fail-loud construction error at assembly time (before goal intake). |
| `AGENT_BUILDER_PLAN_STORE_DIR` | path | `$TMPDIR/agent-builder-planstore` | no | On-disk directory backing the durable, read-gated plan store (ADR 065, task 173), only used when `AGENT_BUILDER_MEMORY_GUARD_BIN` is set. The default lives under the OS temp dir; set this to a persistent path for cross-reboot durability of plans awaiting approval. |
...
| `AGENT_BUILDER_GOAL_MAX_ATTEMPTS` | integer `>= 1` | `3` | no | **`orchestrate` subcommand only** (task 169, ADR 065). The **goal-level** re-plan budget: the maximum number of `Handle` attempts `Orchestrator.RunToCompletion` makes before escalating on exhaustion, folding each terminal sub-goal failure back into the goal text and re-planning. Distinct from `AGENT_BUILDER_MAX_ATTEMPTS`, which bounds each sub-goal's own `runtime.Run` retries one layer below. Unlike the lenient concurrency bounds, a non-integer or `< 1` value is a **fail-fast** `ExitUsage` configuration error (the budget is a correctness bound, not a tuning knob). The attempt counter is persisted in the goal's `runstore.Record` when a run journal is configured, so the budget survives a crash mid-loop. |
| `AGENT_BUILDER_APPROVAL_TIMEOUT` | Go duration `> 0` | `1h` | no | **`orchestrate` subcommand only** (task 171, ADR 065). How long an unresolved sub-goal-level pending approval (task 170) waits before `Orchestrator.SweepApprovalTimeouts` auto-escalates it over the `Reporter` exactly once. Parsed as a Go duration (e.g. `30m`, `2h`); a non-duration, empty, or `<= 0` value falls back to the default (a tuning knob for escalation timing, not a security gate). |
| `AGENT_BUILDER_DAEMON_LOCK` | path | `$TMPDIR/agent-builder-daemon.lock` | no | **`daemon` subcommand only** (task 174, ADR 065). Single-instance lock file. `agent-builder daemon` creates it with `O_CREATE\|O_EXCL` (writing its PID) before starting the control loop; a second concurrent `daemon` against the same path fails fast, non-zero, naming the path, without starting a second loop. Removed on graceful shutdown (SIGINT/SIGTERM) and on any startup-assembly error. No PID-liveness detection: an operator manually clears a stale lock after a hard crash (a documented v1 limitation). |
| `AGENT_BUILDER_SCHEDULE_PATH` | path | unset | no | **`daemon` subcommand only** (task 175, ADR 065). A JSON schedule file of recurring/deferred goals the daemon fires without an inbound message. Each entry sets exactly one of `every` (a Go duration, e.g. `"1h"`) or `at` (a daily time `"HH:MM"`), plus a `goal` text: `{"entries":[{"goal":"nightly build","every":"1h"},{"goal":"morning report","at":"03:00"}]}`. Fired entries route through the SAME control-loop goal-intake path as channel-originated goals (no parallel dispatch), as a `supervisor.Task{ID:"sched-<n>-<RFC3339>", Spec:<goal>}`. A malformed file (unparseable value, both/neither of `every`/`at`, empty goal) is a fail-fast `ExitUsage` error before the loop starts. Unset = no scheduler (zero goroutines). Full cron syntax is a dependency-approval follow-on; schedule-firing state is not persisted across a restart (possible duplicate firing near a restart-adjacent boundary, a documented v1 limitation). |
| `AGENT_BUILDER_RUN_STORE_DIR` | path | unset | no | **`orchestrate`/`daemon` subcommands** (task 167/168, ADR 065). On-disk directory for the durable run journal. When set, the orchestrator persists per-goal run/attempt state (crash-safe append-only JSONL + snapshot), and the `daemon` rehydrates in-flight runs at startup (`RehydrateInFlight`/`ResumeFromRecord`) before entering the steady-state loop. Unset = no durable journaling (byte-for-byte pre-task behavior). A malformed on-disk state is a fail-loud assembly error before goal intake. |
...
| `AGENT_BUILDER_TELEGRAM_AUTH_MODE` | enum: `envelope`, `allowlist`, `pairing`, `open`, `disabled` | `envelope` (same as unset) | no | **`orchestrate` subcommand only** (task 151, ADR 063 Decision 1). Selects the inbound sender-ID auth mode. Unset or `envelope` reproduces today's adapter exactly: every update is verified+opened via `envelope.VerifyAndOpen` and the sender ID is never consulted (the strong-security default; MUST NOT be weakened). `disabled` rejects all inbound traffic before any parse/armor. `allowlist` accepts **plaintext** commands only from statically-seeded approved sender IDs. `pairing` accepts plaintext from approved senders and routes an unknown sender through an owner-approved in-chat flow: the owner (see `AGENT_BUILDER_TELEGRAM_OWNER_ID`) replies `approve <id>` / `deny <id>`, approvals persist to the same `0600` store so they survive restarts (ADR 063 Decisions 3/4). `open` (task 153) accepts **plaintext from any sender ID whatsoever** — no store lookup, no owner gate, no allowlist check at all; it is the intentional footgun mode, reachable **only** by the exact literal, case-sensitive, whitespace-exact value `open` (`"Open"`, `"OPEN"`, `" open"`, `"open "` are all fail-fast unknown-mode errors, never a silent fallback to `envelope` and never an accidental match to `open` — a typo fails closed to a config error). Assembling `orchestrate` with `open` resolved emits a mandatory, unconditional `WARNING`-prefixed line to stderr exactly once, naming the risk ("any account that finds the bot can command it") — the other four modes never emit it. Every other unrecognized value (including near-misses of any recognized mode name) is a fail-fast `ExitUsage` error at assembly (never a nil-adapter panic at first `Next()`); the empty (or whitespace-only) string is treated as unset (⇒ `envelope`), not an unknown value — but a *non-blank* value is matched exactly, with no trimming or case-folding applied. **Plaintext modes (`allowlist`/`pairing`/`open`) forfeit the envelope's end-to-end confidentiality and per-message replay protection** (ADR 063 Decision 2). The SEC-001/002 size caps and audit events are retained unconditionally on every accepted plaintext path. `armor.Guard` is retained too, but only once an armor binary is actually configured via `AGENT_BUILDER_TELEGRAM_ARMOR_BIN` (task 158, ADR 064): these three modes have **no cryptographic authentication gate at all**, so armor is their only content-level defense, and assembling `orchestrate` with one of them resolved and no armor binary configured is a fail-fast `ExitUsage` error at assembly — never a silent fail-open guard. `envelope`/`disabled` modes retain the fail-open default when no armor binary is configured, since the envelope-verify pipeline (Ed25519+X25519+replay-cache) is the load-bearing gate on that path regardless of armor. |
| `AGENT_BUILDER_TELEGRAM_APPROVED_STORE` | file path | none | required when `AGENT_BUILDER_TELEGRAM_AUTH_MODE` is `allowlist` or `pairing` | **`orchestrate` subcommand only** (task 151, ADR 063 Decision 4). Direct path to the persisted, plain-text, `0600` JSON approved-sender store (loaded once at startup so approvals survive restarts; created `0600` if absent). Holds only public numeric sender IDs — no keys, no secrets. A blank path is a fail-fast `ExitUsage` error for any mode that consults sender-ID approval; `envelope`/`disabled`/`open` never read the store and accept a blank path. A missing file is graceful absence (empty approved set); a malformed file is a fail-fast `Load` error so a corrupted store is noticed rather than silently emptied. In `pairing` mode the owner's `approve <id>` adds a sender ID to this store and persists it in-chat, so the approval survives a process restart. Mirrors the direct-path convention of `AGENT_BUILDER_AUDIT_RECORD` / `AGENT_BUILDER_RUN_RECORD`. |
| `AGENT_BUILDER_TELEGRAM_APPROVED_IDS` | comma-separated numeric IDs | empty | no | **`orchestrate` subcommand only** (task 151, ADR 063 Decision 4). The static approved-sender list seeded into `AGENT_BUILDER_TELEGRAM_APPROVED_STORE` at startup in `allowlist` mode. Sender IDs are normalized to canonical numeric form (leading zeros / whitespace stripped) on seeding, so a formatting difference cannot create a duplicate approval or bypass the gate; a non-numeric entry is a fail-fast `ExitUsage` error. Seeding is **additive/union** — it never removes IDs already present in an existing store file, so `pairing` mode can grow the same store in-chat without a later `allowlist` restart wiping it. |
| `AGENT_BUILDER_TELEGRAM_OWNER_ID` | numeric sender ID | none | required when `AGENT_BUILDER_TELEGRAM_AUTH_MODE` is `pairing` | **`orchestrate` subcommand only** (task 152, ADR 063 Decision 3). The numeric Telegram sender ID of the owner who approves/denies pending senders in `pairing` mode. **Only** messages whose sender ID equals this value are honored for the `approve <id>` / `deny <id>` grammar, which is parsed and owner-gated **before** the adapter's command-verb routing — a stranger's `approve <own-id>` is treated as ordinary unapproved input and can never self-approve (the load-bearing anti-self-approval control). Unset/blank in `pairing` mode is a fail-fast `ExitUsage` error (an owner-less pairing mode has no one who can ever approve a sender); a non-numeric value is likewise fail-fast. The value is normalized to the same canonical numeric form as approved-store entries. Ignored by every other mode. In a 1:1 owner DM the owner's chat ID equals this sender ID, so pairing-request notifications are delivered there. The owner ID is also seeded into the approved-sender store at startup (task 159), so the owner's own first plaintext command routes without a self-approval step. |
| `AGENT_BUILDER_TELEGRAM_POLL_BACKOFF` | Go duration (e.g. `1s`, `500ms`) | `1s` (same as unset/blank/unparseable) | no | **`orchestrate` subcommand only** (task 157). The context-aware wait between the adapter's internal `getUpdates` re-poll attempts — applied when a poll batch yields no deliverable message (idle poll, fully-rejected batch) or after a transport failure, so `Adapter.Next()` neither spins nor hammers a failing endpoint (behavior B-040). It does **not** delay delivery when a batch already contains a deliverable message. Unset, blank, or unparseable (including a non-positive duration) falls back to the built-in 1s default. Primarily a test knob (integration tests set `1ms` against a stub server); production rarely needs to override it since the Telegram long-poll itself paces idle polls. |
| `AGENT_BUILDER_TELEGRAM_ARMOR_BIN` | executable path | none | required when `AGENT_BUILDER_TELEGRAM_AUTH_MODE` is `allowlist`, `pairing`, or `open` | **`orchestrate` subcommand only** (task 158, ADR 064). Resolves the armor binary wired as the Telegram adapter's `ContentGuard`, via the same `exec.LookPath` pattern `AGENT_BUILDER_AUDIT_BIN`/`resolveAuditBin` uses. When resolvable, `armor.NewGuard(armor.Config{Command: [resolvedPath]})` is wired for **every** auth mode (`envelope`, `allowlist`, `pairing`, `open`); a configured value that does not resolve to an executable is a fail-fast `ExitUsage` error at assembly. When unset: `envelope`/`disabled` modes retain the pre-task fail-open `allowAllContentGuard` default (the envelope-verify pipeline is the load-bearing gate there, always enforced); `allowlist`/`pairing`/`open` modes — which have no cryptographic authentication gate at all — fail assembly fast instead, since armor is their only content-level defense (see `AGENT_BUILDER_TELEGRAM_AUTH_MODE`'s row above). |
...
AGENT_BUILDER_REGISTRY_CLAUDE_OAUTH_MODEL=claude-opus-4-8
