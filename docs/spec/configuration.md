...
| `AGENT_BUILDER_GOAL_ANALYSIS` | enum string | (unset = off) | no | **`orchestrate` subcommand only** (ADR 060). When enabled (`true`/`1`/`yes`/`heuristic`/`on`, case-insensitive), the orchestrator analyzes each goal at intake and classifies its **kind** (answer vs coding) and **complexity**. A `KindAnswer` goal (a general question, no repo) is **answered over the channel** via the single-shot Completer — routed to a brain at a capability floor derived from complexity — and never dispatched (read-only inference: no worker, gate, branch, spawn-policy, or approval). A `KindCoding` goal follows the existing clarify→plan→gate→approval→dispatch flow. A `KindAnswer` goal is **multi-turn** (ADR 060 §6): after the first reply it lingers in `StateConversing`, and a follow-up `info <goalID> <text>` is answered with the running conversation transcript as context, until `cancel <goalID>` or source EOF ends it. When unset/false (default), every goal is coding (pre-060 behavior). The default analyzer is the deterministic `HeuristicGoalAnalyzer`. |
