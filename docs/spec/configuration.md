...
**Last updated:** 2026-06-28 (task 099 — orchestrate subcommand: added AGENT_BUILDER_PLANNER, AGENT_BUILDER_GOAL_SPEC/_ID/_REPO; orchestrator now on the live binary path)
...
| `AGENT_BUILDER_PLANNER` | enum: `structured`, `llm` | `structured` | no | **`orchestrate` subcommand only** (task 099). Selects the `Planner` the orchestrator assembles. `structured` (default) wires the rule-based `StructuredPlanner` (no LLM, no executor import). `llm` is reserved for the LLM-backed planner (task 100); until that merges it returns a clear "not yet available" error (`ErrPlannerNotAvailable`) at assembly — the subcommand fails fast before any goal intake rather than silently falling back. Any other value is a fail-fast configuration error. |
