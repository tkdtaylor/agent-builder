...
| `AGENT_BUILDER_TELEGRAM_AUTH_MODE` | enum: `envelope`, `allowlist`, `pairing`, `open`, `disabled` | `envelope` (same as unset) | no | **`orchestrate` subcommand only** (task 151, ADR 063 Decision 1). Selects the inbound sender-ID auth mode. Unset or `envelope` reproduces today's adapter exactly: every update is verified+opened via `envelope.VerifyAndOpen` and the sender ID is never consulted (the strong-security default; MUST NOT be weakened). `disabled` rejects all inbound traffic before any parse/armor. `allowlist` accepts **plaintext** commands only from statically-seeded approved sender IDs. `pairing` accepts plaintext from approved senders and routes an unknown sender through an owner-approved in-chat flow: the owner (see `AGENT_BUILDER_TELEGRAM_OWNER_ID`) replies `approve <id>` / `deny <id>`, approvals persist to the same `0600` store so they survive restarts (ADR 063 Decisions 3/4). `open` (task 153) is recognized by config validation but its runtime path lands in a later task. An unrecognized value is a fail-fast `ExitUsage` error at assembly (never a nil-adapter panic at first `Next()`); the empty string is treated as unset (⇒ `envelope`), not an unknown value. **Plaintext modes (`allowlist`/`pairing`/`open`) forfeit the envelope's end-to-end confidentiality and per-message replay protection** — `armor.Guard`, the SEC-001/002 size caps, and audit events are retained on every accepted plaintext path (ADR 063 Decision 2). |
