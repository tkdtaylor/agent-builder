...
**Last updated:** 2026-07-01 (task 152 — Telegram `pairing` mode adds the owner-approved in-chat flow on the sender-ID auth-mode branch: unknown sender → owner-gated `approve`/`deny` (before command routing) → persisted approval that survives restart, plus a new plaintext `PairingNotifier` outbound edge, ADR 063 Decisions 3/4; builds on task 151's `envelope`/`disabled`/`allowlist` branch)
