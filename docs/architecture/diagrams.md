...
**Telegram inbound auth-mode branch (task 151 / ADR 063).** `AGENT_BUILDER_TELEGRAM_AUTH_MODE` inserts an opt-in sender-ID mode decision into `telegram.Adapter.Next()` **ahead of** the envelope pipeline above. The decision is owned by the `internal/channel/telegram/authz` sub-unit (a stdlib-only leaf on the channel side of the `MessageSource` seam — never imports `internal/supervisor`, so F-003/F-007 are unaffected), which also owns the persisted `0600` JSON approved-sender store. Per update, the branch is: **`envelope` (default, unset ⇒ this)** → the untouched `VerifyAndOpen` → armor path above (sender ID ignored); **`disabled`** → reject before any parse/armor (`channel_disabled` audit); **`allowlist`** → the `authz` store's normalized sender-ID membership gates the update — an approved sender's **plaintext** runs the SAME size-cap + `armor.Guard` + `deriveMessage` pipeline (`plaintext_accepted` audit), an unapproved sender is rejected before armor (`sender_not_approved` audit). Plaintext modes forfeit the envelope's confidentiality/replay protection (ADR 063 Decision 2); armor, size caps, and audit stay load-bearing on every accepted plaintext. **`pairing` (task 152)** extends `allowlist`: an approved sender routes identically, while an **unknown** sender enters an owner-approved in-chat flow — `authz.DecidePairing` gates the `approve <id>`/`deny <id>` grammar on the **owner sender ID first** (before the text is inspected, so a stranger's `approve <own-id>` can never self-approve) and, on the owner's `approve`, adds the target to the same persisted `0600` store so the approval survives a restart. The pairing flow's plaintext "pending" reply to the sender and approve/deny notification to the owner go out over a `PairingNotifier` (`PlaintextNotifier`) — a NEW plaintext outbound edge distinct from the envelope-sealing `ReplyAdapter`, since the unknown sender holds no envelope key. **`open` (task 153)** closes out the branch: it accepts plaintext from **any** sender ID with no store lookup and no owner gate at all — the SAME size-cap + `armor.Guard` + `deriveMessage` pipeline runs as `allowlist`/`pairing` (armor and audit are never bypassed), it is reachable only by the exact literal `"open"` (no case-folding, no whitespace tolerance), and resolving to it makes `orchestrate` assembly print one mandatory `WARNING` line to stderr naming the risk. No new box or edge is introduced for `open` — it is another leaf on the same mode-decision branch. The flow arrow into the orchestrator (`goal (supervisor.Task, envelope-verified + armor-guarded)`) is unchanged — the mode branch only widens *how* an update becomes trusted plaintext, not what the supervisor receives.
